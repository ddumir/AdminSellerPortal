        ...
        .then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                ...
                tableRow.innerHTML = `
                    ...
                    <button onclick="rejectSeller('${doc.id}')">Rej</button>
                    </td>
                `;
                    ...
                    firebase.auth().createUserWithEmailAndPassword(sellerEmail, password)
                        .then(() => {
                            ...
                            Email.send({
                                ...
                                Subject: "SavvyO Seller Approved!",
...
// Function to reject a seller application
function rejectSeller(sellerId) {
    console.log('Rejecting seller:', sellerId); // Log the sellerId to see if 
    db.doc(sellerId).update({ status: 'rejected' })
        .then(() => {
            console.log('Seller application rejected successfully');
            // Show a pop-up message
            alert('Seller Rejected!');
            fetchPendingApplications(); // Refresh the list after rejection
        })
        .catch((error) => {
